...
public class lbbroker
{
  ...
  private static class ClientTask extends Thread
  {
    ...
    public void run ()
    {
      try (final Context context = ZMQ.context (1))
      {
        ...
        try (final Socket client = context.socket (ZMQ.REQ))
        {
   ...
   * conceptually acts as a separate process. This is the worker task, using a
   * REQ socket to do load-balancing.
   */
  private static class WorkerTask extends Thread
  {
    ...
    {
      // Prepare our context and sockets
      try (final Context context = ZMQ.context (1); final Socket worker = context.socket (ZMQ.REQ))
      {
        ...
        {
          final String address = worker.recvStr ();
          final String empty = worker.recvStr ();
          assert (empty.length () == 0);

          // Get request, send reply
          final String request = worker.recvStr ();
  ...
  /**
   * This is the main task. It starts the clients and workers, and then routes
   * requests between the two layers. Workers signal READY when they start;
   * after that we treat them as ready when they reply with a response back to a
   * client. The load-balancing data structure is just a queue of next available
   * workers.
   *
   * @param args
   *        ignored
  ...
  public static void main (final String [] args)
  {
    try (final Context context = ZMQ.context (1);
         ...
         final Socket backend = context.socket (ZMQ.ROUTER))
    {
      ...
      final Queue <String> workerQueue = new LinkedList<> ();
      ...
      while (!Thread.currentThread ().isInterrupted ())
      {
        ...
        if (items.pollin (0))
        {
          ...
          final String clientAddr = backend.recvStr ();
        ...
        if (items.pollin (1))
        {
          ...
          final String clientAddr = frontend.recvStr ();
          ...
          final String workerAddr = workerQueue.poll ();
