...
public class clonesrv6
{
  ...
  private static class Snapshots implements IZLoopHandler
  {
    ...
    {
      final clonesrv6 srv = (clonesrv6) arg;
  ...
  private static class SendHugz implements IZLoopHandler
  {
    ...
    public int handle (final ZLoop loop, final PollItem item, final Object arg)
    {
      ...
      final kvmsg msg = new kvmsg (srv.sequence);
      msg.setKey ("HUGZ");
  ...
  private static class NewActive implements IZLoopHandler
  {
    ...
    public int handle (final ZLoop loop, final PollItem item, final Object arg)
    {
      ...
      final PollItem poller = new PollItem (srv.subscriber, ZMQ.Poller.POLLIN);
      srv.bStar.zloop ().removePoller (poller);

      // Apply pending list to own hash table
      for (final kvmsg msg : srv.pending)
      {
        msg.setSequence (++srv.sequence);
        msg.send (srv.publisher);
        msg.store (srv.kvmap);
        System.out.println ("I: publishing pending=" + srv.sequence);
      }
  ...
  private static class NewPassive implements IZLoopHandler
  {
    ...
    public int handle (final ZLoop loop, final PollItem item, final Object arg)
    {
      ...
      if (srv.kvmap != null)
      {
        for (final kvmsg msg : srv.kvmap.values ())
          msg.destroy ();
      }
      srv.active = false;
      srv.passive = true;
  ...
  private static class Subscriber implements IZLoopHandler
  {
    ...
    public int handle (final ZLoop loop, final PollItem item, final Object arg)
    {
      ...
      if (srv.kvmap == null)
      {
        ...
        final Socket snapshot = srv.ctx.createSocket (ZMQ.DEALER);
        snapshot.connect ("tcp://localhost:" + srv.peer);

        System.out.println ("I: asking for snapshot from: tcp://localhost:" + srv.peer);
        snapshot.sendMore ("ICANHAZ?");
        snapshot.send (""); // blank subtree to get all

        while (true)
        {
          final kvmsg msg = kvmsg.recv (snapshot);
          if (msg == null)
            break; // Interrupted
        ...
        System.out.println ("I: received snapshot=" + srv.sequence);
        srv.ctx.destroySocket (snapshot);

      }

      // Find and remove update off pending list
      final kvmsg msg = kvmsg.recv (item.getSocket ());
      if (msg == null)
        return 0;
      ...
      if (!msg.getKey ().equals ("HUGZ"))
      {
        ...
        if (msg.getSequence () > srv.sequence)
        {
          srv.sequence = msg.getSequence ();
          msg.store (srv.kvmap);
          System.out.println ("I: received update=" + srv.sequence);
        }
      }
      msg.destroy ();
  ...
  public clonesrv6 (final boolean bPrimary)
  {
    if (bPrimary)
    {
    ...
    subscriber.connect ("tcp://localhost:" + (peer + 1));
  }
  ...
  public void run ()
  {
    ...
    final PollItem poller = new PollItem (collector, ZMQ.Poller.POLLIN);
    ...
    for (final kvmsg value : pending)
      value.destroy ();

    bStar.destroy ();
    for (final kvmsg value : kvmap.values ())
      value.destroy ();
  ...
  // else return FALSE.
  boolean wasPending (final kvmsg msg)
  {
    final Iterator <kvmsg> it = pending.iterator ();
    while (it.hasNext ())
    {
