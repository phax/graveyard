...
public class mdbroker
{
  ...
  {
    public final String _name; // Service name
  ...
  {
    String _identity;// Identity of worker
    ZFrame _address;// Address frame to route to
  ...
  private boolean _verbose = false;// Print activity to stdout
  ...
  {
    final mdbroker broker = new mdbroker (args.length > 0 && "-v".equals (args[0]));
    ...
    while (!Thread.currentThread ().isInterrupted ())
    {
      ...
      if (items.pollin (0))
      {
        ...
        if (MDP.C_CLIENT.frameEquals (header))
        {
          ...
          if (MDP.W_WORKER.frameEquals (header))
  ...
  private void processWorker (final ZFrame sender, final ZMsg msg)
  {
    ...
    if (MDP.W_READY.frameEquals (command))
    {
      ...
      if (MDP.W_REPLY.frameEquals (command))
      {
        if (workerReady)
        {
          ...
          msg.addFirst (worker.service._name);
  ...
  private void deleteWorker (final Worker worker, final boolean disconnect)
  {
    ...
    workers.remove (worker._identity);
    worker._address.destroy ();
  }
  ...
  public synchronized void purgeWorkers ()
  {
    ...
    {
      log.format ("I: deleting expired worker: " + w._identity);
   ...
   */
  private void dispatch (final Service service, final ZMsg msg)
  {
    assert (service != null);
    ...
    {
      final ZMsg msg2 = service.requests.pop ();
      final Worker worker = service.waiting.pop ();
      waiting.remove (worker);
      sendToWorker (worker, MDP.W_REQUEST, null, msg2);
      msg2.destroy ();
    }
  }
   ...
   */
  public void sendToWorker (final Worker worker, final MDP command, final String option, final ZMsg msgp)
  {
    ...
    msg.wrap (worker._address.duplicate ());
    if (_verbose)
    {
