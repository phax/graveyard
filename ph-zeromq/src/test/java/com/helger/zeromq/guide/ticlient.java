...
{
  static ZMsg serviceCall (final mdcliapi session, final String service, final ZMsg request)
  {
    final ZMsg reply = session.send (service, request);
    if (reply != null)
    {
      final ZFrame status = reply.pop ();
      if (status.streq ("200"))
      {
        status.destroy ();
        return reply;
  ...
  public static void main (final String [] args) throws Exception
  {
    ...
    final mdcliapi session = new mdcliapi ("tcp://localhost:5555", verbose);
    ...
    while (!Thread.currentThread ().isInterrupted ())
    {
      ...
      if (reply != null)
      {
        final String replyString = reply.getLast ().toString ();
