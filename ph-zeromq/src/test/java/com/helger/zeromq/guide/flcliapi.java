...
public class flcliapi
{
  ...
  {
    private final String _endpoint; // Server identity/endpoint
  ...
  {
    private final Socket _pipe; // Socket to talk back to application
  ...
  static private class FreelanceAgent implements IAttachedRunnable
  {
    ...
    public void run (final Object [] args, final ZContext ctx, final Socket pipe)
    {
      ...
      final PollItem [] items = { new PollItem (agent._pipe, ZMQ.Poller.POLLIN),
      ...
      while (!Thread.currentThread ().isInterrupted ())
      {
        ...
        final int rc = ZMQ.poll (items, (tickless - System.currentTimeMillis ()));
        if (rc == -1)
