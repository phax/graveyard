  ...
  public static void main (final String [] argv)
  {
    ...
    try (final ZContext ctx = new ZContext ())
    {
      ...
      while (true)
      {
        ...
        final PollItem backends[] = { new PollItem (localbe, Poller.POLLIN), new PollItem (cloudbe, Poller.POLLIN) };
        // If we have no workers anyhow, wait indefinitely
        int rc = ZMQ.poll (backends, capacity > 0 ? 1000 : -1);
        if (rc == -1)
          break; // Interrupted
        ...
        while (capacity > 0)
        {
          final PollItem frontends[] = { new PollItem (localfe, Poller.POLLIN), new PollItem (cloudfe, Poller.POLLIN) };
