...
public class ppworker
{
  ...
  private static Socket worker_socket (final ZContext ctx)
  {
  ...
  public static void main (final String [] args)
  {
    ...
    while (true)
    {
      final PollItem items[] = { new PollItem (worker, ZMQ.Poller.POLLIN) };
      final int rc = ZMQ.poll (items, HEARTBEAT_INTERVAL);
      if (rc == -1)
        break; // Interrupted
      ...
      if (items[0].isReadable ())
      {
        ...
        if (--liveness == 0)
        {
          ...
          System.out.println ("W: reconnecting in " + interval + " msec");
          try
          {
