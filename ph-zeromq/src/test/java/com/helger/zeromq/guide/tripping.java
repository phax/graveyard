...
public class tripping
{
  ...
  static class Client implements Runnable
  {
    ...
    public void run ()
    {
      try (final ZContext ctx = new ZContext ())
      {
        ...
        {
          final ZMsg req = new ZMsg ();
          req.addString ("hello");
          req.send (client);
          ZMsg.recvMsg (client).destroy ();
        }

        System.out.printf (" " + (1000 * SAMPLE_SIZE) / (System.currentTimeMillis () - start) + " calls/second");
        ...
        System.out.println (" " + (1000 * SAMPLE_SIZE) / (System.currentTimeMillis () - start) + " calls/second");
      }
    }
  ...
  public static void main (final String [] args)
  {
    ...
    final Thread brokerThread = new Thread (new Broker ());
    final Thread workerThread = new Thread (new Worker ());
    final Thread clientThread = new Thread (new Client ());
