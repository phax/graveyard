    ...
    try (final ZMQ.Context context = ZMQ.context (1))
    {
      ...
      try (final ZMQ.Socket subscriber = context.socket (ZMQ.SUB))
      {
        ...
        for (update_nbr = 0; update_nbr < 100; update_nbr++)
        {
          ...
          final int zipcode = Integer.parseInt (sscanf.nextToken ());
          final int temperature = Integer.parseInt (sscanf.nextToken ());
          @SuppressWarnings ("unused")
          final int relhumidity = Integer.parseInt (sscanf.nextToken ());
