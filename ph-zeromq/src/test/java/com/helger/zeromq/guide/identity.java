...
public class identity
{
  ...
  {
    try (final Context context = ZMQ.context (1); final Socket sink = context.socket (ZMQ.ROUTER))
    {
      sink.bind ("inproc://example");

      // First allow 0MQ to set the identity, [00] + random 4byte
      try (final Socket anonymous = context.socket (ZMQ.REQ))
      {
        ...
        try (final Socket identified = context.socket (ZMQ.REQ))
        {
