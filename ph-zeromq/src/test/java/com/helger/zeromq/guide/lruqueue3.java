  ...
  public void run ()
  {
    try (final ZContext context = new ZContext ())
    {
      ...
      while (true)
      {
        ...
        if (data == null)
          break;
        final String reply = new String (data, ZMQ.CHARSET);
        try
        {
          Thread.sleep (1000);
        }
        ...
        System.out.println (Thread.currentThread ().getName () + " Client Sent HELLO - " + reply);
      }
    }
  }
}
...
class FrontendHandler implements ZLoop.IZLoopHandler
{
  ...
  public int handle (final ZLoop loop, final PollItem item, final Object arg_)
  {
    ...
    if (msg != null)
    {
      ...
      {
        final PollItem poller = new PollItem (arg.frontend, ZMQ.Poller.POLLIN);
