...
public class kvmsg
{
  ...
  {
    final ByteBuffer msg = ByteBuffer.allocate (props_size);
    for (final Entry <Object, Object> o : props.entrySet ())
    {
      final String prop = o.getKey ().toString () + "=" + o.getValue ().toString () + "\n";
  ...
  {
    final byte [] msg = frame[FRAME_PROPS];
    ...
    for (final String prop : new String (msg, ZMQ.CHARSET).split ("\n"))
    {
  ...
  public kvmsg (final long sequence)
  {
  ...
  // new {{kvmsg}} instance:
  public static kvmsg recv (final Socket socket)
  {
    // This method is almost unchanged from kvsimple
    // .skip
    assert (socket != null);
    final kvmsg self = new kvmsg (0);
    ...
    for (frameNbr = 0; frameNbr < KVMSG_FRAMES; frameNbr++)
    {
      ...
      final boolean rcvmore = (frameNbr < KVMSG_FRAMES - 1) ? true : false;
  ...
  public String getKey ()
  {
    if (present[FRAME_KEY])
    {
      if (_key == null)
      {
        ...
        _key = new String (buf, ZMQ.CHARSET);
      }
      return _key;
    }
    return null;
  ...
  {
    final byte [] msg = new byte [key.length ()];
  ...
  public void fmtKey (final String fmt, final Object... args)
  {
  ...
  public long getSequence ()
  {
    if (present[FRAME_SEQ])
    {
      ...
      final ByteBuffer source = ByteBuffer.wrap (frame[FRAME_SEQ]);
  ...
  {
    final ByteBuffer msg = ByteBuffer.allocate (8);
  ...
  {
    final byte [] msg = new byte [body.length];
  ...
  {
    setBody (body.getBytes (ZMQ.CHARSET));
  }

  // Set message body using printf format
  public void fmtBody (final String fmt, final Object... args)
  {
    setBody (String.format (fmt, args));
  }
  ...
  public void setUUID ()
  {
    final byte [] msg = UUID.randomUUID ().toString ().getBytes (ZMQ.CHARSET);
  ...
  // value is 255 chars.
  public void setProp (final String name, final String fmt, final Object... args)
  {
    setProp (name, String.format (fmt, args));
  }
  ...
  {
    final Object old = props.setProperty (name, value);
    if (old != null)
  ...
  public void store (final Map <String, kvmsg> hash)
  {
    if (size () > 0)
    {
  ...
  public void dump ()
  {
    ...
    System.err.print ("[seq:" + getSequence () + "]");
    System.err.print ("[getKey:" + getKey () + "]");
    // .until
    System.err.print ("[size:" + size + "] ");
    System.err.print ("[");
    for (final String key : props.stringPropertyNames ())
    {
      System.err.print (key + "=" + props.getProperty (key) + ";");
    }
    System.err.print ("]");

    // .skip
    for (int charNbr = 0; charNbr < size; charNbr++)
      System.err.print (StringHelper.getHexStringLeadingZero2 (body[charNbr]));
    System.err.print ("\n");
  }
  ...
  public void test (final boolean verbose)
  {
    ...
    {
      final Socket output = ctx.createSocket (ZMQ.DEALER);
      output.bind ("ipc://kvmsg_selftest.ipc");
      final Socket input = ctx.createSocket (ZMQ.DEALER);
      ...
      kvmsg = com.helger.zeromq.guide.kvmsg.recv (input);
      if (verbose)
        kvmsg.dump ();
      ...
      if (verbose)
        kvmsg.dump ();
      assert (kvmsg._key.equals ("getKey"));
      assert (kvmsg.getProp ("prop2").equals ("value2"));
      kvmsg.destroy ();
