...
public class bstar
{
  ...
  private static IZLoopHandler SendState = (loop, item, arg) -> {
    final bstar self = (bstar) arg;
    self.statepub.send (Integer.toString (self.state.ordinal ()));
    return 0;
  };

  // Receive state from peer, execute finite state machine
  private static IZLoopHandler RecvState = (loop, item, arg) -> {
    final bstar self = (bstar) arg;
    final String state = item.getSocket ().recvStr ();
    if (state != null)
    {
  ...
  private static IZLoopHandler VoterReady = (loop, item, arg) -> {
    final bstar self = (bstar) arg;
  ...
  public bstar (final boolean primary, final String local, final String remote)
  {
    ...
    final PollItem poller = new PollItem (statesub, ZMQ.Poller.POLLIN);
  ...
  public int voter (final String endpoint, final int type, final IZLoopHandler handler, final Object arg)
  {
    // Hold actual handler+arg so we can call this later
    final Socket socket = ctx.createSocket (type);
  ...
  public void newActive (final IZLoopHandler handler, final Object arg)
  {
    activeFn = handler;
    activeArg = arg;
  }

  public void newPassive (final IZLoopHandler handler, final Object arg)
  {
    passiveFn = handler;
    passiveArg = arg;
  }
