...
public class bstarsrv
{
  ...
  public static void main (final String [] argv)
  {
    ...
    {
      final Socket statepub = ctx.createSocket (ZMQ.PUB);
      final Socket statesub = ctx.createSocket (ZMQ.SUB);
      statesub.subscribe (ZMQ.SUBSCRIPTION_ALL);
      final Socket frontend = ctx.createSocket (ZMQ.ROUTER);
      final bstarsrv fsm = new bstarsrv ();
      ...
      {
        final PollItem [] items = { new PollItem (frontend, ZMQ.Poller.POLLIN),
                                    new PollItem (statesub, ZMQ.Poller.POLLIN), };
        int timeLeft = (int) ((sendStateAt - System.currentTimeMillis ()));
        if (timeLeft < 0)
          timeLeft = 0;
        final int rc = ZMQ.poll (items, 2, timeLeft);
        if (rc == -1)
        ...
        if (items[0].isReadable ())
        {
          ...
          if (fsm.stateMachine () == false)
                                           // Answer client by echoing request
                                           // back
                                           msg.send (frontend);
          else
            msg.destroy ();
        ...
        if (items[1].isReadable ())
        {
          ...
          final String message = statesub.recvStr ();
