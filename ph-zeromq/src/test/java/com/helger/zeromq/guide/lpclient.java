    ...
    try (final ZContext ctx = new ZContext ())
    {
      ...
      while (retriesLeft > 0 && !Thread.currentThread ().isInterrupted ())
      {
        ...
        while (expect_reply > 0)
        {
          ...
          final PollItem items[] = { new PollItem (client, Poller.POLLIN) };
          final int rc = ZMQ.poll (items, REQUEST_TIMEOUT);
          if (rc == -1)
            break; // Interrupted
          ...
          if (items[0].isReadable ())
          {
            ...
            {
              System.out.println ("I: server replied OK (" + reply + ")");
              retriesLeft = REQUEST_RETRIES;
              expect_reply = 0;
            }
            else
              System.out.println ("E: malformed reply from server: " + reply);
