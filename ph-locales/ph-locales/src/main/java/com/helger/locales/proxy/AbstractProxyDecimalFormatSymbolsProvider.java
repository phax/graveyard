...
/**
 * An implementation of {@link DecimalFormatSymbolsProvider} that proxies the
 * methods to the implementations of another locale.
 *
 * @author Philip Helger
 */
public abstract class AbstractProxyDecimalFormatSymbolsProvider extends DecimalFormatSymbolsProvider
{
  private final Set <Locale> m_aSupportedLocales = new LinkedHashSet <Locale> ();
  private final Locale m_aProxyLocale;

  public AbstractProxyDecimalFormatSymbolsProvider (final Collection <Locale> aSupportedLocales,
                                                    final Locale aProxyLocale)
  {
  ...
  @Override
  public DecimalFormatSymbols getInstance (final Locale aLocale)
  {
    if (aLocale == null)
      throw new NullPointerException ();

    if (m_aSupportedLocales.contains (aLocale))
      return DecimalFormatSymbols.getInstance (m_aProxyLocale);
