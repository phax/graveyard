...
/**
 * An implementation of {@link DateFormatSymbolsProvider} that proxies the
 * methods to the implementations of another locale.
 *
 * @author Philip Helger
 */
public abstract class AbstractProxyDateFormatSymbolsProvider extends DateFormatSymbolsProvider
{
  private final Set <Locale> m_aSupportedLocales = new LinkedHashSet <Locale> ();
  private final Locale m_aProxyLocale;

  public AbstractProxyDateFormatSymbolsProvider (final Collection <Locale> aSupportedLocales, final Locale aProxyLocale)
  {
  ...
  public DateFormatSymbols getInstance (final Locale aLocale)
  {
    ...
    if (m_aSupportedLocales.contains (aLocale))
      return DateFormatSymbols.getInstance (m_aProxyLocale);
