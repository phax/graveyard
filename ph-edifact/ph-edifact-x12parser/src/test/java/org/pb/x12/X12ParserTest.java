...
public class X12ParserTest
{
  ...
  public static final String EXPECTED_X12_TOSTRING = "ISA*00*          *00*          *ZZ*SENDERID       *ZZ*RECEIVERID    *030409*0701*U*00401*0000000001*0*T*:~" +
  ...
  public static final String EXPECTED_X12_TOXML = "<LOOP NAME=\"X12\">" +
                                                  "<LOOP NAME=\"ISA\">" +
  ...
  {
    final IX12Parser parser = new X12Parser (_loadCf ());
    final URL url = this.getClass ().getResource ("/example/example835One.txt");
    final File f1 = new File (url.getFile ());

    final X12 x12 = (X12) parser.parse (f1);

    assertEquals (EXPECTED_X12_TOSTRING, x12.getAsString ());
  ...
  public void testParseInputStream () throws EDIFormatException, IOException
  {
    ...
    final X12 x12 = (X12) parser.parse (is);
  ...
  {
    final String inputString = EXPECTED_X12_TOSTRING;

    final IX12Parser parser = new X12Parser (_loadCf ());
    final X12 x12 = (X12) parser.parse (inputString);
